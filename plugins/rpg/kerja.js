...
let handler = async (m, { conn, command, args, usedPrefix }) => {
     ...
     let time = db.data.users[m.sender].lastkerja + 72000000
     ...
     if (/kerjadulu|kerja|work/i.test(command)) {
          switch (type) {
               case 'pembunuhbayaran':
                    if (db.data.users[m.sender].pembunuhbayaran == false) throw 'Ini Bukan Tugas Kamu Atau Kamu Pengangguran!'
                    if (new Date - db.data.users[m.sender].lastkerja < 72000000) throw `Kamu Sudah Bekerja\nSaatnya Istirahat Selama ${clockString(time - new Date())}`
                    db.data.users[m.sender].eris += coinpb
                    ...
                    if (pecatPB > 1) {
                         db.data.users[m.sender].pembunuhbayaran -= pecatPB * 1
                         m.reply(ppecatPB)
                    }
                    break
               case 'mencaritanamanobat':
                    if (db.data.users[m.sender].mencaritanamanobat == false) throw 'Ini Bukan Tugas Kamu Atau Kamu Pengangguran!'
                    if (new Date - db.data.users[m.sender].lastkerja < 72000000) throw `Kamu Sudah Bekerja,Saatnya Istirahat Selama\n${clockString(time - new Date())}`
                    db.data.users[m.sender].eris += coinmto
                    db.data.users[m.sender].lastkerja = new Date * 1
                    m.reply(`Kamu Menemukan Tanahamn Obat *${mencaritanamanobattt}* dan Menjualnya\nDan Mendapatkan Eris Senilai *${coinmto}*`)
                    if (pecatMTO > 1) {
                         db.data.users[m.sender].mencaritanamanobat -= pecatMTO * 1
                         m.reply(ppecatMTO)
                    }
                    break
               case 'pengantarbarang':
                    if (db.data.users[m.sender].pengantarbarang == false) throw 'Ini Bukan Tugas Kamu Atau Kamu Pengangguran!'
                    if (new Date - db.data.users[m.sender].lastkerja < 72000000) throw `Kamu Sudah Bekerja,Saatnya Istirahat Selama\n${clockString(time - new Date())}`
                    db.data.users[m.sender].eris += coinpgb
                    db.data.users[m.sender].lastkerja = new Date * 1
                    m.reply(`Kamu Mengantar *${pengantarbaranggg}*\nDan Mendapatkan Eris Senilai *${coinpgb}*`)
                    if (pecatPGB > 1) {
                         db.data.users[m.sender].pengantarbarang -= pecatPGB * 1
                         m.reply(ppecatPGB)
                    }
                    break
               case 'petani':
                    if (db.data.users[m.sender].petani == false) throw 'Ini Bukan Tugas Kamu Atau Kamu Pengangguran!'
                    if (new Date - db.data.users[m.sender].lastkerja < 72000000) throw `Kamu Sudah Bekerja,Saatnya Istirahat Selama\n${clockString(time - new Date())}`
                    db.data.users[m.sender].eris += coinptn
                    db.data.users[m.sender].lastkerja = new Date * 1
                    m.reply(`${panen} Sudah Panen Dan Menjualnya\nHasil Menjual Mendapatkan Eris Senilai *${coinptn}*`)
                    if (pecatPTN > 1) {
                         db.data.users[m.sender].petani -= pecatPTN * 1
                         m.reply(ppecatPTN)
                    }
                    break
               case 'penjualmakanan':
                    if (db.data.users[m.sender].penjualmakanan == false) throw 'Ini Bukan Tugas Kamu Atau Kamu Pengangguran!'
                    if (new Date - db.data.users[m.sender].lastkerja < 72000000) throw `Kamu Sudah blBekerja,Saatnya Istirahat Selama\n${clockString(time - new Date())}`
                    db.data.users[m.sender].cointembaga += coinpm
                    db.data.users[m.sender].lastkerja = new Date * 1
                    m.reply(`Kamu Baru Saja Membuat Makanan *${penjualmakanannn}* Untuk Pelanggan\nDan Mendapatkan Coin Tembaga Senilai *${coinpm}*`)
                    break
               case 'pelatih':
                    if (db.data.users[m.sender].pelatih == false) throw 'Ini Bukan Tugas Kamu Atau Kamu Pengangguran!'
                    if (new Date - db.data.users[m.sender].lastkerja < 72000000) throw `Kamu Sudah Bekerja,Saatnya Istirahat Selama\n${clockString(time - new Date())}`
                    db.data.users[m.sender].eris += coinplt
                    db.data.users[m.sender].lastkerja = new Date * 1
                    m.reply(`Kamu Baru Saja Selesai ${pelatihhh}\nDan Mendapatkan Eris Senilai *${coinplt}*`)
                    if (pecatPLT > 1) {
                         db.data.users[m.sender].pelatih -= pecatPLT * 1
                         m.reply(ppecatPLT)
                    }
                    break
               default:
